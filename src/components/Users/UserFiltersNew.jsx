/**
 * Filtros actualizados para usuarios con soporte para perfiles
 * Extiende UserFilters con nuevas opciones de filtrado
 */

import React, { useMemo } from 'react';
import PropTypes from 'prop-types';
import { UniversalFilters } from '../Common';

const UserFiltersNew = ({
  searchTerm = '',
  onSearchChange,
  selectedRole = '',
  onRoleChange,
  selectedTipoUsuario = '',
  onTipoUsuarioChange,
  selectedPerfil = '',
  onPerfilChange,
  selectedDateRange = null,
  onDateRangeChange,
  roles = [],
  perfiles = [],
  onCreateNew,
  onExport,
  onImport,
  totalItems = 0
}) => {
  // Configuración de filtros actualizada
  const filtersConfig = useMemo(() => {
    return {
      searchConfig: {
        key: 'searchTerm',
        placeholder: 'Buscar por nombre, DNI, email, empresa...',
        variant: 'default'
      },
      
      filterGroups: [
        {
          key: 'selectedTipoUsuario',
          title: 'Tipo de Usuario',
          type: 'dropdown',
          options: [
            { value: '', label: 'Todos los tipos' },
            { value: 'ADMIN', label: 'Administrativos' },
            { value: 'TRABAJADOR', label: 'Trabajadores' }
          ],
          placeholder: 'Seleccionar tipo...',
          showIcon: true,
          iconPath: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'
        },
        {
          key: 'selectedPerfil',
          title: 'Perfil',
          type: 'dropdown',
          options: [
            { value: '', label: 'Todos los perfiles' },
            ...perfiles.map(perfil => ({
              value: perfil.id?.toString() || perfil.nombre,
              label: perfil.nombre || perfil.label
            }))
          ],
          placeholder: 'Seleccionar perfil...',
          showIcon: true,
          iconPath: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'
        },
        {
          key: 'selectedRole',
          title: 'Rol Legacy',
          type: 'dropdown',
          options: [
            { value: '', label: 'Todos los roles' },
            ...roles.map(role => ({ 
              value: role.name || role.id?.toString(), 
              label: role.name || role.label 
            }))
          ],
          placeholder: 'Seleccionar rol...',
          showIcon: true,
          iconPath: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z'
        },
        {
          key: 'selectedDateRange',
          title: 'Fecha Registro',
          type: 'dateRange',
          placeholder: 'Seleccionar rango...',
          showIcon: true,
          iconPath: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'
        }
      ],
      
      actions: [
        {
          label: 'Crear',
          variant: 'primary',
          icon: 'M12 6v6m0 0v6m0-6h6m-6 0H6',
          onClick: onCreateNew
        },
        {
          label: 'Exportar',
          variant: 'secondary',
          icon: 'M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
          onClick: onExport
        },
        {
          label: 'Importar',
          variant: 'secondary',
          icon: 'M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12',
          onClick: onImport,
          isFileInput: true
        }
      ],
      
      itemLabel: 'usuarios'
    };
  }, [roles, perfiles, onCreateNew, onExport, onImport]);

  // Estado actual de filtros
  const activeFilters = {
    searchTerm,
    selectedRole,
    selectedTipoUsuario,
    selectedPerfil,
    selectedDateRange
  };

  // Manejar cambios de filtros
  const handleFilterChange = (filterKey, value) => {
    switch (filterKey) {
      case 'searchTerm':
        onSearchChange?.(value);
        break;
      case 'selectedRole':
        onRoleChange?.(value);
        break;
      case 'selectedTipoUsuario':
        onTipoUsuarioChange?.(value);
        break;
      case 'selectedPerfil':
        onPerfilChange?.(value);
        break;
      case 'selectedDateRange':
        onDateRangeChange?.(value);
        break;
      default:
        break;
    }
  };

  // Limpiar todos los filtros
  const handleClearFilters = () => {
    onSearchChange?.('');
    onRoleChange?.('');
    onTipoUsuarioChange?.('');
    onPerfilChange?.('');
    onDateRangeChange?.(null);
  };

  return (
    <UniversalFilters
      searchConfig={filtersConfig.searchConfig}
      filterGroups={filtersConfig.filterGroups}
      actions={filtersConfig.actions}
      activeFilters={activeFilters}
      onFilterChange={handleFilterChange}
      onClearFilters={handleClearFilters}
      totalItems={totalItems}
      itemLabel={filtersConfig.itemLabel}
      className="-mt-4 mb-6"
    />
  );
};

UserFiltersNew.propTypes = {
  searchTerm: PropTypes.string,
  onSearchChange: PropTypes.func,
  selectedRole: PropTypes.string,
  onRoleChange: PropTypes.func,
  selectedTipoUsuario: PropTypes.string,
  onTipoUsuarioChange: PropTypes.func,
  selectedPerfil: PropTypes.string,
  onPerfilChange: PropTypes.func,
  selectedDateRange: PropTypes.object,
  onDateRangeChange: PropTypes.func,
  roles: PropTypes.array,
  perfiles: PropTypes.array,
  onCreateNew: PropTypes.func,
  onExport: PropTypes.func,
  onImport: PropTypes.func,
  totalItems: PropTypes.number
};

export default UserFiltersNew;