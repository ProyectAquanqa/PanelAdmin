...
const specialtySchema = z.object({
  name: z.string()
    ...
    .regex(/^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s\-\.]+$/, 'Solo se permiten letras, espacios, guiones y puntos')
    .refine(val => val.trim().length > 0, 'El nombre es requerido'),
  description: z.string()
    .min(10, 'La descripción debe tener al menos 10 caracteres')
    .max(500, 'La descripción no puede exceder 500 caracteres')
    .refine(val => val.trim().length > 0, 'La descripción es requerida'),
...
function SpecialtyFormModal({ isOpen, onClose, specialty = null }) {
  ...
  const onSubmit = async (data) => {
    try {
      // Preparar datos limpiando espacios en blanco
      const specialtyData = {
        name: data.name.trim(),
